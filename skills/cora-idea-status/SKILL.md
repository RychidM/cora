...
description: >
  ...
  when the user says an idea has progressed or stalled. Trigger phrases include:
...
Map the user's phrasing to the closest status. If a target section
...
- **Status change that moves the block** between sections
  (e.g. Active → Shipped, Active → Parked): show the user what will move, then
  apply.
