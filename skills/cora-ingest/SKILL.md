...
description: >
  ...
  `research/sources/` into the user's memory vault, extracting key claims and
  updating relevant topic synthesis pages in `research/topics/`. Use
  this skill when the user asks to ingest, process, file, or summarise a
...
Identify the source file from the user's request:
- If the user names a file: use `research/sources/{path}`
- If the user says "the article I just clipped" or similar: list
  `research/sources/articles/` and use the most recently modified file
- If the user doesn't name one and there's no obvious recent source: ask
...
Wait for the user's response or implicit go-ahead before writing. If the user
says "go ahead" or doesn't object, proceed.

This step is what makes the ingest collaborative — the user stays in
...
3. **Do NOT** edit the project file from this skill — that's the
   job of `cora-logger` + the user's review. Instead, surface the
   connection in the report so the user can decide whether to log it.
...
- **Always discuss before writing** — Step 2 isn't optional. It's how
  the user stays in control of the synthesis.
...
- **Don't edit project files.** Surface project relevance in the
  report; let the user decide whether to log it.
