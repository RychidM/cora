...
description: >
  ...
  Use when RM says a project changed state. Trigger phrases include:
...
none, stop and tell RM the project isn't in the vault.
...
Map RM's phrasing to the closest status.
...
Archiving is a relocation, never a deletion. Confirm with RM before
moving, then:
...
5. Note that any synced repo agent files (CLAUDE.md, etc.) are now stale;
   mention this so RM can re-run or stop syncing.

If the move can't be done with available tools (no shell/move), tell RM
...
If archived, remind RM that repo agent files for that project are now
stale.
