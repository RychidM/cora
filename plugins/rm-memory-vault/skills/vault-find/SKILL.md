...
name: vault-find
description: >
  Searches RM's memory vault for entries by keyword or topic. Returns a
  concise list of matches with their file paths and relevant snippets.
  Use when RM asks to find, search, look up, or recall something from
...
(projects, ideas, brand) so RM can navigate quickly.
...
- **Don't paraphrase the source content** — show actual snippets so RM
  knows what's there
