...
name: cora-resume
description: >
  Picks up a past chat session from the user's memory vault inside a fresh chat,
  with no project context assumed. Lists recent sessions across every
  scope, lets the user pick one, then loads it in full so work can continue.
  Use this skill when the user starts a new chat and wants to resume, pick up,
...
`cora-recall` is the project-bound door: it auto-detects the current
project from the working directory and filters sessions to that scope.
That assumes you're already working inside a project. `cora-resume` is
...
The contract:
- **carry** writes a session out (`cora-carry`)
- **recall** pulls sessions back *for the current project* (`cora-recall`)
...
Resolve vault root (see `cora-logger` Step 1). If no `sessions/`
folder exists, tell the user there are no sessions yet and stop.
...
|------------|------|--------------|
| `/cora-resume` (no args) | **browse** | Lists recent sessions across **all** scopes, then loads the one the user picks |
| `/cora-resume <query>` | **match** | Matches a session by slug or keyword; loads it directly if unique, else lists candidates to pick from |

Flags:
- `--limit N` — how many sessions browse mode lists (default 5, max 15)
- `--include-archived` — also consider `sessions/_archive/`

Unlike `cora-recall`, there is **no scope filter and no project
...
Wait for the user to pick. Then load that session in full via Step 5.

If there are no sessions at all: say so plainly and stop.

---

## Step 4 — Match mode (`/cora-resume <query>`)
...
3. Resolve:
   ...
   - **Several matches** → list them as in Step 3 and ask the user to pick
   - **No match** → say so, then fall back to listing the most recent
     sessions (Step 3) so the user can pick from what exists
...
- If the session has decisions worth logging: *"Want to log any of these
  decisions to pending review? (`cora-logger`)"*
- If the session has carried references worth promoting: *"Want to
  ingest the references into the research library? (`cora-ingest`)"*
- If the resumed work produces new artifacts by the end of this chat:
  *"Want to carry this session forward when you're done? (`cora-carry`)"*
...
- **No scope assumption.** Resume never auto-detects a project or
  filters by scope — that's `cora-recall`'s job. Browse mode spans
  every scope so it works from a blank chat.
- **Browse, then load.** Default behaviour lists recent sessions and
  waits for the user to pick before loading anything in full.
