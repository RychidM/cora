---
name: cora-project-init
description: >
  Initialises a new project or module in the user's memory vault. Creates the
  ...
  the user asks to create, init, add, or set up a new project or module.
...
If the template doesn't exist, stop and ask the user.
...
2. **Delete `{target_path}/ACTIVITY.md`.** Modules never get their own —
   their activity rolls up into the parent's feed via `cora-logger`
   breadcrumbs.
...
stays empty until the project gets its first module and a `cora-logger`
...
2. Invoke `cora-project-sync` (or instruct the user to run it)
...
Next:
  ...
  Run /cora-sync if you haven't already
```
...
- **Never invent placeholder content** — leave template `*(Fill in)*`
  placeholders alone for the user to complete
