...
description: >
  ...
  when the user asks to carry, capture, save, or close out a chat session.
...
References to carry (N):
  ...
  - Carried:    Article excerpt the user pasted earlier → references/apple-apns-notes.md
...
Wait for the user's response or implicit go-ahead. Adjust based on feedback.
...
For each code block or diagram the user confirmed in Step 3:
...
For each reference the user confirmed to carry as a file:

- External text the user pasted (article excerpt, PM feedback, etc.) →
  `references/{descriptive-slug}.md`
- A repo file's content the user dropped into chat for context →
...
- **Always discuss before writing.** Step 3 is mandatory. The carry
  reflects the user's emphasis, not the skill's interpretation alone.
- **Single scope per session.** No multi-scope lists. If a session
  genuinely spans two surfaces, ask the user to split it.
...
- **Never auto-promote.** Sessions don't get auto-logged or
  auto-ingested. Surface candidates in the report; let the user decide.
- **Never modify project files.** A session about a project doesn't
  ...
  session belongs in a project file, the user uses `cora-logger` for it.
