...
name: cora-recall
description: >
  Loads recent chat sessions from the user's memory vault back into the current
  agent context. Use this skill when the user asks to load, recall, pull,
...
The opposite direction from `cora-carry`: carry IN, recall OUT.
...
|------------|------|--------------|
| `/cora-recall` (no args) | **list** | Lists recent sessions for the **current project** (auto-detected) |
| `/cora-recall <scope-prefix>` | **list** | Lists recent sessions matching that scope prefix |
| `/cora-recall full <slug>` | **full** | Loads one session completely (SESSION.md + references; artifacts on request) |
...
If the user gave an explicit scope prefix, use it. Otherwise auto-detect:
...
Drill into one with: `/cora-recall full <slug>`
```
...
When invoked as `/cora-recall full <slug>`:
...
- **Honour the archive boundary.** List mode excludes
  `sessions/_archive/` by default. To include archived sessions, the user
  invokes `/cora-recall <scope> --include-archived`.
