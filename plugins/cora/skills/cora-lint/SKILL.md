...
name: cora-lint
description: >
  Health-checks the user's memory vault for contradictions, stale claims,
  ...
  writes. Use when the user asks to lint, audit, check, or health-check the
...
A diagnostic pass over the vault. Produces a structured report so the user
can decide what to fix. Never writes — fixes are the user's call.
...
- **Missing feed** — a top-level project folder with `OVERVIEW.md` but
  ...
  was created without going through `cora-project-init`.
...
includes the destination path so the user can drag-and-drop in Obsidian or
run a shell `mv`.
...
- `projects/some-standalone-project/` (top-level) has `OVERVIEW.md` but
  no `ACTIVITY.md`
  *Suggested action: scaffold it from the template (predates ACTIVITY.md being added, or wasn't created via cora-project-init)*
...
- `projects/agentwatch/ACTIVITY.md` newest entry 2026-03-15; module
  ...
  *Suggested action: activity isn't being logged — confirm writes go through cora-logger*
...
- **Suggest, don't decide.** Every finding gets a "Suggested action"
  line; the user decides what to act on.
