...
The opposite direction from `cora-carry`.

| | |
|---|---|
| **Command** | `/cora-recall` |
| **Mode** | Read-only |
| **SKILL.md** | `skills/cora-recall/SKILL.md` |
...
|------------|-----------|
| `/cora-recall` | Lists recent sessions for the current project (auto-detected from cwd) |
| `/cora-recall <scope-prefix>` | Filters by explicit scope (prefix match) |
| `/cora-recall full <slug>` | Loads one session completely |
...
[cora-resume](cora-resume.md) — the scope-agnostic counterpart: pick up
any session in a fresh chat when you're not working inside a project.
[cora-carry](cora-carry.md) — creates the sessions this skill loads.
[cora-find](cora-find.md), [cora-read](cora-read.md) — general
search and read; `cora-recall` is the sessions-specific version with
scope filtering and lean defaults.
[cora-logger](cora-logger.md) — for promoting a decision from a
loaded session.
[cora-ingest](cora-ingest.md) — for promoting a carried reference
