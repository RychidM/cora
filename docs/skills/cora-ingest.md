...
| **SKILL.md** | `skills/cora-ingest/SKILL.md` |
...
- **Never edits project files** — project relevance is surfaced in the
  report so the user can choose to log it through `cora-logger`.

## Key rules

- **Always discusses proposed topic targets before writing** (Step 2 of
  the skill). The user stays in control of emphasis.
...
[cora-lint](cora-lint.md) — periodic health check; flags source
