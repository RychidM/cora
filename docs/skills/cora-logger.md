...
| **SKILL.md** | `skills/cora-logger/SKILL.md` |
...
3. **Propose the full draft inline** — every touched file shown, batched
   into one proposal.
4. **Write only after the user's explicit approval**, in one pass.
...
[cora-edit](cora-edit.md). The write protocol mirrors the vault's
