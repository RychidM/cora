...
| **SKILL.md** | `skills/cora-lint/SKILL.md` |
...
- `/cora-lint orphans` — only the orphan-pages check
- `/cora-lint stale` — only the stale-topics check
- `/cora-lint drift` — only the AGENTS.md projects-table check
...
- **Writes:** nothing. Fixes are the user's call.
...
- **Every finding has a Suggested action line** — the user still decides.
- **Caps long lists** — checks finding >20 items show the top 10 and
  note how many more.

## Related

[cora-ingest](cora-ingest.md) — produces the topic pages this lint
checks for staleness and conflicts.
[cora-carry](cora-carry.md) — produces the sessions this lint checks
for staleness.
[cora-status](cora-status.md) — lighter-weight read of current vault
