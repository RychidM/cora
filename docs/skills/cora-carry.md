...
| **SKILL.md** | `skills/cora-carry/SKILL.md` |
...
Inferred from the conversation. Confirmed with the user in a proposal step
...
- **Discusses before writing** (Step 3) — the user controls slug, scope, and
...
[cora-recall](cora-recall.md) — the counterpart skill that loads
sessions back into agent context for the current project.
[cora-resume](cora-resume.md) — picks up any session in a fresh chat
with no project context assumed.
[cora-lint](cora-lint.md) — flags stale active sessions (>60 days)
for manual archival.
[cora-find](cora-find.md), [cora-read](cora-read.md) — discover and
read past sessions.
[cora-logger](cora-logger.md) — for promoting a single decision from a
session into the pending review log.
[cora-ingest](cora-ingest.md) — for promoting a carried reference into
